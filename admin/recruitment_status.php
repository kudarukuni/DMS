					...
					<div class="card-body">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$types = $conn->query("SELECT * FROM enrollment_status where status = 1 order by id asc");
...
<script>
	...
	$('.delete_scat').click(function(){
		_conf("Are you sure to delete this enrollment status category","delete_scat",[$(this).attr('data-id')])
	})
